import * as React from "react";
import type { SVGProps } from "react";
const SvgDeviceGamepad3Icon = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width="1em"
    height="1em"
    fill="none"
    viewBox="0 0 24 24"
    {...props}
  >
    <g
      stroke="currentColor"
      strokeLinecap="round"
      strokeLinejoin="round"
      strokeWidth={2}
      clipPath="url(#device-gamepad-3-icon_svg__a)"
    >
      <path d="M10 12 7 9H5a1 1 0 0 0-1 1v4a1 1 0 0 0 1 1h2zM14 12l3-3h2a1 1 0 0 1 1 1v4a1 1 0 0 1-1 1h-2zM12 14l-3 3v2a1 1 0 0 0 1 1h4a1 1 0 0 0 1-1v-2zM12 10 9 7V5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2z" />
    </g>
    <defs>
      <clipPath id="device-gamepad-3-icon_svg__a">
        <path fill="#fff" d="M0 0h24v24H0z" />
      </clipPath>
    </defs>
  </svg>
);
export default SvgDeviceGamepad3Icon;
